//CPC In-Market Bid Modifer Script
...
function main() {
  ...
  Logger.log('Getting campaign CPC');
  var campaignCPC = getCampaignCPC();
  
  Logger.log('Getting audiences CPC');
  var listOfAudiencesCPC = getAudienceCPC();
  
  Logger.log('Making operations');
  var operations = makeAllOperations(audienceMapping, campaignCPC, listOfAudiencesCPC);
...
//RUNS 2
//returns campaign level performance for Cost Per Click
function getCampaignCPC(){
  var campaignCPC = {};
  var query = "SELECT CampaignName, AverageCpc FROM CAMPAIGN_PERFORMANCE_REPORT WHERE AverageCpc > 0 DURING " + DATE_RANGE;
  var rows = AdsApp.report(query).rows();

  while (rows.hasNext()) {
    var row = rows.next();
    var averageCpc = row.AverageCpc;
    averageCpc = parseFloat(averageCpc.replace(",", ""));
    campaignCPC[row['CampaignName']] = averageCpc;
  }
  return campaignCPC;
}

//RUNS 3
//returns audience level performance for Cost Per Click
function getAudienceCPC(){
  var listOfAudiencesCPC = [];
  var query = "SELECT Criteria, AverageCpc, CampaignName FROM AUDIENCE_PERFORMANCE_REPORT WHERE AverageCpc > 0 DURING " + DATE_RANGE;
  var rows = AdsApp.report(query).rows();
  ...
  while (rows.hasNext()) {
    ...
    var averageCpc = row.AverageCpc;
    averageCpc = parseFloat(averageCpc.replace(",", ""));
    var audienceObject = {campaignName: row['CampaignName'], audienceID: criteriaSplit[1], CPC: averageCpc, uniqueID: uniqueID}
    listOfAudiencesCPC.push(audienceObject);
  }
  return listOfAudiencesCPC;
}
...
function makeAllOperations(audienceMapping, campaignCPC, listOfAudiencesCPC) {
  var operations = [];
  ...
  while (filteredCampaigns.hasNext()) { //iterates over each campaign
    ...
    var operationsFromCampaign = makeOperationsFromEntity(campaign, campaignCPC[campaign.getName()], audienceMapping, "Campaign", listOfAudiencesCPC); //for each item in the campaign performance list, it takes the CPA
...
function makeOperationsFromEntity(campaign, campaignCPC, audienceMapping, levelApplyingAt, listOfAudiencesCPC) {
  var entityAudiences = getAudiencesFromEntity(campaign, audienceMapping); //Get all audiences for each campaign
  return makeOperations(campaignCPC, entityAudiences, campaign.getName(), levelApplyingAt, listOfAudiencesCPC);
}
...
function makeOperations(campaignCPC, audiences, entityName, entityType, listOfAudiencesCPC) {
  var operations = [];
  ...
  audiences.forEach(function (audience) {
    ...
    var arrayLength = listOfAudiencesCPC.length;
    for (var i = 0; i < arrayLength; i++) {
      if (listOfAudiencesCPC[i].uniqueID === uniqueID){
        var audienceCPC = listOfAudiencesCPC[i].CPC;
        var modifier = (campaignCPC / audienceCPC)
