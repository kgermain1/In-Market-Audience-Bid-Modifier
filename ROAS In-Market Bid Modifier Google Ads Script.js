//ROAS In-Market Bid Modifer Script
...
function main() {
  ...
  Logger.log('Getting campaign ROAS');
  var campaignROAS = getCampaignROAS();
  
  Logger.log('Getting audiences ROAS');
  var listOfAudiencesROAS = getAudienceROAS();
  
  Logger.log('Making operations');
  var operations = makeAllOperations(audienceMapping, campaignROAS, listOfAudiencesROAS);
...
//RUNS 2
//returns campaign level performance for ConversionValue and Cost and calculates ROAS
function getCampaignROAS(){
  var campaignROAS = {};
  var query = "SELECT CampaignName, ConversionValue, Cost FROM CAMPAIGN_PERFORMANCE_REPORT WHERE ConversionValue > 0 AND Cost > 0 DURING " + DATE_RANGE;
  ...
  while (rows.hasNext()) {
    ...
    campaignROAS[row['CampaignName']] = parseFloat(conversionValue/cost);
  }
  return campaignROAS;
}

//RUNS 3
//returns audience level performance for ConversionValue and Cost and calculates ROAS
function getAudienceROAS(){
  var listOfAudiencesROAS = [];
  var query = "SELECT Criteria, ConversionValue, CampaignName, Cost FROM AUDIENCE_PERFORMANCE_REPORT WHERE ConversionValue > 0 AND Cost > 0 DURING " + DATE_RANGE;
  var rows = AdsApp.report(query).rows();
  ...
  while (rows.hasNext()) {
    ...
    conversionValue = parseFloat(conversionValue.replace(",", ""));
    cost = parseFloat(cost.replace(",", ""));
    var audienceObject = {campaignName: row['CampaignName'], audienceID: criteriaSplit[1], ROAS: parseFloat(conversionValue/cost), uniqueID: uniqueID, Cost: cost}
    listOfAudiencesROAS.push(audienceObject);
  }
  return listOfAudiencesROAS;
}
...
function makeAllOperations(audienceMapping, campaignROAS, listOfAudiencesROAS) {
  var operations = [];
  ...
  while (filteredCampaigns.hasNext()) { //iterates over each campaign
    ...
    var operationsFromCampaign = makeOperationsFromEntity(campaign, campaignROAS[campaign.getName()], audienceMapping, "Campaign", listOfAudiencesROAS); //for each item in the campaign performance list, it takes the CPA
...
function makeOperationsFromEntity(campaign, campaignROAS, audienceMapping, levelApplyingAt, listOfAudiencesROAS) {
  var entityAudiences = getAudiencesFromEntity(campaign, audienceMapping); //Get all audiences for each campaign
  return makeOperations(campaignROAS, entityAudiences, campaign.getName(), levelApplyingAt, listOfAudiencesROAS);
}
...
function makeOperations(campaignROAS, audiences, entityName, entityType, listOfAudiencesROAS) {
  var operations = [];
  ...
  audiences.forEach(function (audience) {
    ...
    var arrayLength = listOfAudiencesROAS.length;
    for (var i = 0; i < arrayLength; i++) {
      if (listOfAudiencesROAS[i].uniqueID === uniqueID){
        var audienceROAS = listOfAudiencesROAS[i].ROAS;
        Logger.log(campaignName + " " + audienceID)
        Logger.log("audience ROAS: "+audienceROAS)
        Logger.log("campaign ROAS: "+campaignROAS)
        var modifier = (audienceROAS / campaignROAS)
